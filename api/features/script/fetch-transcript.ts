...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!caption_tracks || caption_tracks.length === 0) {
        ...
        // 1. 한국어 자막 찾기 (수동 -> 자동 순)
        let selectedTrack = caption_tracks.find(t => t.language_code === 'ko' && t.kind !== 'asr');
        if (!selectedTrack) {
        ...
        // 2. 한국어 없으면 영어 자막 찾기
        if (!selectedTrack) {
            selectedTrack = caption_tracks.find(t => t.language_code === 'en' && t.kind !== 'asr');
            if (!selectedTrack) {
        ...
        console.log(`[fetch-transcript] Selected track: ${selectedTrack.name.toString()} (${selectedTrack.language_code})`);

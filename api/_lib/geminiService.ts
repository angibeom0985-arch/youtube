...
위 구조의 흐름만 참고하여, "${newKeyword}"를 주제로 완전히 새로운 정보와 내용을 창작해주세요. 모든 결과 항목을 지정된 구조에 맞춰 JSON 형식으로 제공해주세요.`;
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        systemInstruction:
          "당신은 창의적인 YouTube 스크립트 작가 겸 기획자입니다. 성공 공식을 바탕으로 새로운 주제에 대한 기획안을 생성합니다. 요청된 카테고리와 영상 길이에 맞춰 결과물의 형식과 분량을 조절해주세요. 모든 텍스트는 평문으로 작성하고, 마크다운 특수문자(*, **, _, __, #, - 등)를 절대 사용하지 마세요. 대본과 대사는 자연스러운 구어체로 작성하고, 문단 구분은 두 번의 줄바꿈으로 하세요. 긴 영상의 경우 충분한 분량의 대본을 생성해주세요.",
        responseMimeType: "application/json",

...
const CreditPurchasePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // 페이지 메타데이터
    document.title = "크레딧 충전 - 유튜브 제작 스튜디오";
    ...
    updateMetaTag("og:title", "크레딧 충전 - 유튜브 제작 스튜디오");
    updateMetaTag(
      "og:description",
      "매월 든든 자동할 필요가 없습니다. 필요한 때, 필요한 만큼 충전하세요."
    );
      ...
      <main className="max-w-7xl mx-auto px-4 py-12">
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-gray-400">
            매월 든든 자동할 필요가 없습니다.{" "}
            <span className="text-red-300 font-bold">필요한 때</span>,{" "}
            <span className="text-red-300 font-bold">필요한 만큼</span> 충전하세요
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-6">
          {pricingPlans.map((plan) => (
            ...
            >
              ...
              {plan.popular && (
                <p className="text-center text-xs text-gray-400 mt-3">가성비 최고</p>
              )}
            </div>
          ))}
        </div>
        ...
        <div className="mt-16 text-center text-gray-500 text-sm">
          ...
          <p className="mt-2">
            환불 정책은 구매일로부터 7일 이내, 미사용 크레딧에 한해 가능합니다.
          </p>
        </div>
      </main>
    </div>
